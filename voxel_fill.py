...
def collect_bounds(brushes:List[Brush]):
    ...
    for b in brushes:
        ...
        xs.extend([xmin,xmax]);ys.extend([ymin,ymax]);zs.extend([zmin,zmax])
...
def write_mpz(filename,grid,textures,spawn):
    slots=len(textures)
    header=struct.pack('<4sii7i4s',b'MAPZ',MAP_VERSION,44,WORLD_SIZE,1,0,0,slots,GAME_VERSION,MAP_REVISION,GAME_ID)
    variables=struct.pack('<i',0)
    texmru=struct.pack('<H',slots)+b''.join(struct.pack('<H',i) for i in range(slots))
    ent=struct.pack('<3fB3B',*spawn,3,0,0,0)+struct.pack('<i7i',7,*([0]*7))+struct.pack('<i',0)
    vslots=b''.join(struct.pack('<ii',0,-1) for _ in range(slots))
...
def main():
    ...
    offset=(snap(WORLD_SIZE/2-(xmin+xmax)/2), snap(WORLD_SIZE/2-(ymin+ymax)/2), snap(WORLD_SIZE/2-(zmin+zmax)/2))
    ...
    write_mpz('valve220_room_fill.mpz',grid,textures,spawn)
    write_cfg('valve220_room_fill.cfg',textures)
